...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
    // ваш код здесь
    meanSpeedWalk:=meanSpeed(action,duration)*kmhInMsec
    return (walkingCaloriesWeightMultiplier * weight + (math.Pow(meanSpeedWalk,2) / (height/cmInM)) * walkingSpeedHeightMultiplier * weight) * duration * minInH
}
